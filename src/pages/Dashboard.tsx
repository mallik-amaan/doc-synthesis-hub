...
import { FileText, FileCheck, AlertTriangle, TrendingUp, Plus, Clock, CheckCircle2, XCircle, Eye, Scissors } from 'lucide-react';
                  ...
                  <div key={i} className="flex items-center justify-between p-3 rounded-md border border-border hover:bg-accent transition-colors">
                    <div className="flex items-center gap-3">
                      <div className={`h-8 w-8 rounded-md flex items-center justify-center ${
                        ['completed', 'completed_no_gdrive', 'completed_gdrive_failed', 'reviewed'].includes(req.status) ? 'bg-success/8' :
                        req.status === 'failed' ? 'bg-destructive/8' :
                        req.status === 'approved' || req.status === 'review' ? 'bg-warning/8' :
                        'bg-primary/8'
                      }`}>
                        {['completed', 'completed_no_gdrive', 'completed_gdrive_failed', 'reviewed'].includes(req.status) ? <CheckCircle2 className="h-4 w-4 text-success" /> :
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {req.status === 'reviewed' && <span className="badge-success text-xs">Reviewed</span>}
